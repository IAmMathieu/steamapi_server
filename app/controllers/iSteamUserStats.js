const iSteamUserStats = {
  async getGlobalAchievementPercentagesForApp(req, res) {
    ...
    const globalAchPerFetch = await fetch(
      `http://api.steampowered.com/ISteamUserStats/GetGlobalAchievementPercentagesForApp/v0002/?gameid=${gameId}`,
      { method: "GET" }
    );
  ...
  async getPlayerAchievements(req,, res) {
    ...
    const playerAchievementsFetch = await fetch(`http://api.steampowered.com/ISteamUserStats/GetPlayerAchievements/v0001/?appid=${appId}&key=${process.env.STEAM_API_KEY}&steamid=${steamId}`, { method: "GET" });
  ...
  async getUserStatsForGame(req, res) {
    ...
    const getUserStatsForGameFetch = await fetch(`http://api.steampowered.com/ISteamUserStats/GetUserStatsForGame/v0002/?appid=${appId}&key=${process.env.STEAM_API_KEY}&steamid=${steamId}`, { method: "GET" });
